...
class AdminController extends Controller
{
	...
	public function admin_dashboard(request $request)
	{
        ...
        $data['usdtongn2'] = Setting::where('id', 1)->first()->rate_2;
        $data['usdtongn3'] = Setting::where('id', 1)->first()->rate_3;
    ...
    public function update_rate2(request $request)
	{
        Setting::where('id', 1)->update(['rate_2' => $request->rate]);

        return back()->with('message', "Rate Update Successfully");

    }

    public function update_rate3(request $request)
    {
        Setting::where('id', 1)->update(['rate_3' => $request->rate]);
    ...
    public function update_cost2(request $request)
	{
        Setting::where('id', 1)->update(['margin_2' => $request->cost]);

        return back()->with('message', "Cost Update Successfully");

    }

    public function update_cost3(request $request)
    {
        Setting::where('id', 1)->update(['margin_3' => $request->cost]);
    ...
    public function update_user(request $request)
	{
        ...
        if($request->trade == 'credit'){
            ...
            $message = $user->email . "| just debited by |  NGN " . number_format($request->amount) . " | on OPRIME VERIFY";
            send_notification($message);
    ...
    public function update_user_custom(request $request)
	{


        $user = User::whereId($request->id)->update(['custom_price' => $request->custom_price]);


        return back()->with('message', 'User custom price updated');
    ...
    public function approve_payment(request $request)
	{
       ...
       $message = $email . "| Manual Payment  Approved |  NGN " . number_format($request->amount) . " | on OPRIME VERIFY";
